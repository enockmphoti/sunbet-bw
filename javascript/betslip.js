...
// " style="background-color: #1c2431; margin: 10% auto; padding: 25px; border-radius: 5px; box-shadow: 0 4px 8px rgba(0,0,0,0.3); width: 90%; max-width: 500px; position: relative; color: #fff; border: 1px solid #3e4758;"
...
<div
  ...
  "
>
  <div class="screen-load-wrapper">
    <div class="is-forgot single-form-block">
      <div sm-data="closer" class="close-button-form close">
...
function updateBetslips(betslips, paginationDiv) {
  // Get the container
  const container = document.querySelector(
    ".withdrawal-history-wrapper-4.is-betslip"
  );

  // Get the first row (non-header) as template
  const templateRow = document.querySelector(
    ".withdrawal-info-row-grid-2.is-betslip:not(.is-heading)"
  );
  ...
  const tableWrapper = document.querySelector(
    ".withdrawal-history-wrapper-4.is-background-grid.is-betslip"
  );

  let messageWrapper = document.querySelector(".no-transactions-message");
  if (!messageWrapper) {
    // Only create once, donâ€™t spam DOM
    messageWrapper = document.createElement("div");
    messageWrapper.classList.add("no-transactions-message");

    const messageElement = document.createElement("p");
    messageElement.textContent = "No Transactions found";

    messageWrapper.appendChild(messageElement);
    tableWrapper.parentElement.appendChild(messageWrapper);
  }

  if (betslips.length === 0) {
    tableWrapper.style.display = "none";
    messageWrapper.classList.remove("is-hidden");
    templateRow.classList.add("is-hidden");
  } else {
    tableWrapper.style.display = "";
    messageWrapper.classList.add("is-hidden");
    templateRow.classList.remove("is-hidden");
  }
  ...
  const oldPagination = container.querySelector(".betslip-pagination-controls");
  if (oldPagination) oldPagination.remove();

  if (container && paginationDiv) {
    container.appendChild(paginationDiv); // Always last child
  }
...
function createPaginationControls(totalItems) {
  ...
  paginationDiv.className = "betslip-pagination-controls";
  ...
  const maxVisiblePages = window.innerWidth > 767 ? 7 : 2;
  ...
  if (currentPage > 1) {
    ...
    prevBtn.className = "pagination-block";
  ...
  if (currentPage < totalPages) {
    ...
    nextBtn.className = "pagination-block";
...
function initializeModal() {
  ...
  if (!document.getElementById("providerDetailsModal")) {
    ...
    const closeBtn = modal.querySelector("[sm-data=closer]");
...
function loadBetslips(startDate, endDate, skip, take) {
  // If passed as string, parse as local MM-DD-YYYY
  const parseIfNeeded = (date) => {
  ...
  const start = parseIfNeeded(startDate);
  const end = parseIfNeeded(endDate);

  // Set time to start and end of day
  start.setHours(0, 0, 0, 0);
  end.setHours(23, 59, 59, 999);

  console.log("LoadBetslips: ", start.toISOString(), end.toISOString());
  ...
  const queryOptions = {
    ...
    skip: skip || 0, // Ensure these are numbers
    take: take || itemsPerPage,
  };

  // First: get total count for accurate pagination
  simlBC.getWalletTrxCount(queryOptions, (err, countData) => {
    if (err) {
      console.error("Error fetching betslip count:", err);
      // Graceful fallback: try loading page anyway
    }

    const totalItems = countData?.totalItems || 0;
    console.log("Total items for pagination:", totalItems, countData);

    const paginationDiv = createPaginationControls(totalItems);

    // Then: get paginated betslip data
    simlBC.getWalletTrxs(queryOptions, (err, data) => {
      if (err) {
        console.error("Error fetching betslips:", err);
        if (err.errors && err.errors[0]) {
          if (err.errors[0].code === "not-found") {
            updateBetslips([]);
          }
        }
        return;
      ...
      if (data && data.items) {
        ...
        updateBetslips(data.items, paginationDiv);
      } else {
        console.warn("No betslips data returned");
        updateBetslips([]);
      }
    });
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fromDateInput.value = formatDate(firstOfMonth);
  ...
  // Set initial from/to vars
  fromDate = firstOfMonth;
  globalStartDate = fromDate;
  toDate = today;
  globalEndDate = toDate;
  // console.log("Defaults -> From:", fromDate, "To:", toDate);
...
function displayBetslipDetails(data) {
    ...
    console.log("dataSlip",profileData)
  ...
  // Basic info
  const transactionDate = new Date(data.transactionDate).toLocaleString();
  const provider = data.details?.providerName || "Unknown";
  const gameCode = data.details?.gameCode || "";
  const gameName = data.description || data.details?.gameName || "Unknown";
  const amount = data.amount || 0;
  
  const wagerId = data.id || "N/A";
  //const remoteId = data.details?.remoteTransactionId || "N/A";
  //const currency = data.details?.currency || "BWP";


const betDescription = data.description || "N/A";
//const odds = data.odds || "N/A";
const market = data.betAction?.additionalData?.market || "N/A";

// Player ID FIX
const playerId = profileData.player.id || "N/A";
  ...
  betslipDetails.innerHTML = `
    <div class="betslip-details">
      ...
      <p><b>Operator:</b> Sunbet Pty Ltd</p>
      <p><b>Address:</b> <span id="operatorAddress"></span></p>
      <p><b>Player ID:</b> ${playerId}</p>
      <p><b>Wager ID:</b> ${wagerId}</p>
      <p><b>Date/Time:</b> ${transactionDate}</p>
      <p><b>Game:</b> ${gameName}</p>
      <p><b>Bet Description:</b> ${betDescription}</p>
      <p><b>Provider:</b> ${provider}</p>
      <p id="licenseNumber"><b>License Number:</b> <span class="loading-text">Loading...</span></p>
      <p><b>Amount:</b> ${amount}</p>
      <p><b>Wager Type:</b> Single (Fixed Odds)</p>
      
    </div>

    <div class="action-buttons">
      <a href="javascript:void(0)" id="exportPdf" data-id="${wagerId}" class="action-button">Export PDF</a>
    </div>
  `;

  betslipDetails.style.display = "block";

  // Copy Wager ID (Optional button logic if present)
  const copyWagerIdBtn = document.getElementById("copyWagerId");
  if (copyWagerIdBtn) {
    copyWagerIdBtn.addEventListener("click", () => {
      navigator.clipboard.writeText(wagerId);
      ...
      setTimeout(() => {
        copyWagerIdBtn.src = "/content/dam/sunbet/images/copy-regular.svg";
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener("click", () => {
      exportBetslipPDF(wagerId); // Assuming exportBetslipPDF is defined globally
    });
  }

  // Format date for provider lookup
  const dateObj = new Date(data.transactionDate);
  const transactionDateISO = isNaN(dateObj.getTime())
    ? new Date().toISOString()
    : dateObj.toISOString();

  // Get provider license info
  simlBC.getProviderDetails(provider,transactionDateISO, gameCode, (err, providerData) => {
    if (err) {
      console.error("Error fetching provider details:", err);
      document.getElementById("licenseNumber").innerHTML =
        "<b>License Number:</b> <span>Not available</span>";
      document.getElementById("operatorAddress").innerText =
        "Shop 17, Riverside Junction, Emnotweni Avenue Riverside Park, Mbombela, Ehlanzeni";
      saveBetslipDataToLocalStorage(data, "Not available", "Fallback address");
      return;
    }

    console.log("prpviderData :",providerData);

    const licenseNumber = providerData?.licenseNumber || "Not available";
    const address =
      providerData?.physical_address ||
      "Shop 17, Riverside Junction, Emnotweni Avenue Riverside Park, Mbombela, Ehlanzeni";

    document.getElementById("licenseNumber").innerHTML =
      `<b>License Number:</b> <span>${licenseNumber}</span>`;
    document.getElementById("operatorAddress").innerText = address;
