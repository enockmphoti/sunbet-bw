...
function displaySlotHistory() {
    ...
    pageData.forEach((item) => {
      ...
      row.innerHTML = `
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.trxid || "-"}</div>
        </div>
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.reqDTM || "-"}</div>
        </div>
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.ref}</div>
        </div>
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.amount}</div>
        </div>
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.complete}</div>
        </div>
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.status}</div>
        </div>
        <div class="withdrawal-mobile-flex table-cell">
          <div class="is-no-wrap">${item.msg}</div>
        </div>`;
  ...
  function attemptFetch() {
    try {
      simlBC.getMyZakaHistory(0, function (err, response) {
        if (err) {
        ...
        if (response && response.length > 0) {
          ...
          noDataRow.innerHTML =
            '<div class="withdrawal-mobile-flex table-cell">No deposit history available</div>';
