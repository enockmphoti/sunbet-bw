...
function initializeMessages() {
  ...
  simlBC.listMessages({}, (err, response) => {
    ...
    response.items.forEach((message, index) => {
      ...
      messageElement.addEventListener("click", (e) => {
        ...
        // Add click handler for delete anchor
      const deleteAnchor = document.querySelector('[data-message="delete"]');
      if (deleteAnchor) {
        deleteAnchor.addEventListener('click', function(e) {
          ...
          console.log('Delete anchor clicked for message id:', messageId);
        ...
        simlBC.getMessage(messageId, (err, messageData) => {
          //console.log("getMessage data for messageId", messageId, ":", messageData);
          if (err) {
  ...
  if (!response.items || response.items.length === 0) {
    document.querySelector('.messages-inbox-content-heading-flex').style.display = 'none';
    const messageContainer = document.querySelector('[data-message="message"]');

    if (messageContainer) {
        messageContainer.innerHTML = '<p>No new messages</p>';
    }
        return;
      }
    });
...
document.querySelector(".nav-letter-svg").addEventListener('click', function() {
  setTimeout(initializeMessages, 10);
});

document.addEventListener('DOMContentLoaded', function() {
   if (simlBC.isLoggedIn()){
          simlBC.getMessageCounts({}, function(err, data) {
        if (data != undefined) {
          var messageCount = data["totalUnreadCount"];
          console.log("Unread messages count:", messageCount);
          var totalCount = data["totalCount"];
          console.log("Message data received:", data);
          $('#messagecount').attr('data-total-count', totalCount);
            if(messageCount > 0){
            document.querySelector('[sunbet="message-count"]').innerHTML = messageCount;
            console.log("Unread messages count: " + messageCount);
            // Reset count to 0 when message icon is clicked
            document.querySelector(".nav-letter-svg").addEventListener("click", function() {
              document.querySelector('[sunbet="message-count"]').innerHTML = "0";
            });
            }
            else{
            document.querySelector('[sunbet="message-count"]').innerHTML = "0";
