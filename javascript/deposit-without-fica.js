(async () => {
  ...
  // Promo popup
document.querySelector('[sunbet-deposit="promo-modal-trigger"]').addEventListener("click", () => {
  Swal.fire({
    icon: "question",
    iconHtml:
      '<img src="https://cdn.prod.website-files.com/66956eb2aafafe3229e15ef3/66f06e639fcc1c59489d26cd_TAG%20SVG%202.svg">',
    ...
    html: `
      <div style="width:100%;">
        <input id="promo-input" class="swal2-input" placeholder="Enter Promo Code" maxlength="20" style="text-transform: uppercase;width:100%;" />
        <div id="promo-error" style="color:#e74c3c;font-size:0.875rem;display:none;margin-top:-10px;"></div>
      </div>
    `,
    ...
    didOpen: () => {
      ...
      inputEl.addEventListener("input", (e) => {
        ...
        if (!value) {
          showPromoError("Promo code is required.");
          confirmBtn.disabled = true;
        } else if (!/^[A-Z-]+$/.test(value)) {
          showPromoError("Only uppercase A–Z letters and hyphens allowed.");
          confirmBtn.disabled = true;
        } else {
    ...
    preConfirm: async () => {
      const inputEl = document.getElementById("promo-input");
      const promoCode = inputEl.value.trim().toUpperCase();

      const error = validatePromoCode(promoCode);
      if (error) {
        showPromoError(error);
        return false;
      }

      const result = await promoPreConfirm(promoCode);
      if (result?.error) {
        Swal.fire({
          icon: "error",
          title: "Oops...",
          text: result.error.message || "There was an error activating your promotion code.",
          showCloseButton: true
        });
        return false;
      }

      promoAfterConfirm(result);
      return false;
    }
  });
});
...
function validatePromoCode(code) {
  ...
  if (!/^[A-Z-]+$/.test(code)) return "Only A–Z letters and hyphens allowed.";
  if (!/^[A-Z][A-Z0-9-]*$/.test(code)) return "Must start with a letter.";
  return null;
}

async function promoPreConfirm(promoCode) {
  const SIGNUP = "SIGNUPPageBonus17";
  const MVG = "MVGClaimBurn16";
  const DEP = "DEP-";

  const isSignup = promoCode.includes(SIGNUP);
  const isMVG = promoCode.includes(MVG);
  const isDeposit = promoCode.includes(DEP);

  if (isSignup || isMVG) {
    return {
      error: { message: "This promo code type cannot be used here." }
    };
  }

  if (!isSignup && !isMVG && !isDeposit) {
    return await new Promise((resolve) => {
      simlBC.activatePromo(promoCode, (err, data) => {
        if (err) {
          console.log("Promo Activation Error:", err);
          const errorMessage =
            err?.errors?.[0]?.detail ||
            err?.errors?.[0]?.title ||
            err?.message ||
  ...
  if (isDeposit) {
    return await new Promise((resolve) => {
      simlBC.getSegment((err, segments) => {
        if (err) {
          console.log("Segment Fetch Error:", err);
          return resolve({
            error: {
              message: err?.message || "Error fetching segment data."
            }
          });
        }

        const found = segments.find((s) => s.name === promoCode);
        if (found) {
          simlBC.activateDepositBonus(found.id, function (err) {
            if (err) {
              console.log("Deposit Bonus Activation Error:", err);
              return resolve({
                error: {
                  message:
                    err?.errors?.[0]?.detail ||
                    err?.message ||
                    "Failed to activate deposit bonus."
                }
              });
            } else {
              resolve({ data: found });
          ...
          resolve({ error: { message: "Invalid promo code." } });
        }
      });
    });
  }

  return { error: { message: "Unhandled promo logic." } };
}

function promoAfterConfirm(result) {
  if (result?.error) {
    Swal.fire({
      icon: "error",
      title: "Oops...",
