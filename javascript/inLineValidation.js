...
function validateMobile(field, errorDiv) {
  ...
  if (value === "") {
        ...
        if (digits.length === 11 && digits.startsWith("267")) {
          ...
          errorMessage =
            digits.length < 8
              ? "Please enter a valid Botswana mobile number (267 + 8 digits)"
...
function validateDOBField(value, fieldName) {
  ...
  if (day && month && year) {
    ...
    if (
      age < 21 ||
      (age === 21 && monthDiff < 0) ||
      (age === 21 && monthDiff === 0 && today.getDate() < date.getDate())
    ) {
      return { isValid: false, message: "Must be 21 or older" };
    }
  }
